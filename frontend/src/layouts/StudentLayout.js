...
/**
 * StudentLayout - sidebar + topbar shell for all student pages.
 * Receives 'children' (page content) and 'title' (topbar heading).
 */
function StudentLayout({ children, title }) {
  ...
  const navLinks = [
    ...
    { path: '/student/skills',         label: 'Skills',         icon: '⚡' },
    { path: '/student/certifications', label: 'Certifications', icon: '🏆' },
    { path: '/student/internships',    label: 'Internships',    icon: '🏢' },
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className="w-60 bg-white border-r border-slate-100 flex flex-col flex-shrink-0">
        ...
        <div className="px-5 py-5 border-b border-slate-100 flex items-center gap-2.5">
          ...
          <div>
            <p className="text-blue-600 font-black text-base leading-none tracking-tight">MentR</p>
            <p className="text-slate-400 text-[10px] font-medium mt-0.5">Student Portal</p>
          </div>
        </div>
